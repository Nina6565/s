...
window.onload = function () {
	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		scoreText.color = 'rgba(117,124,256,1)';		//色　RGB+透明度　今回は白
		...
		//ぞう山ボタン
		const zoyamaImg = new Sprite(100, 106);				//画像サイズをここに書く。使う予定の画像サイズはプロパティで見ておくこと
		...
		game.onenterframe = function () {
			...
			if (state == 3) {							//状態３（point６以上から）
				zoyamaImg.x += 5;
				zoyamaImg.y = 200 + Math.sin(zoyamaImg.x / 70) * 200; // ｙ座標を振幅100pxのサイン波で移動(Sinは便利なので慣れとくといいよ！)
			}
			...
			//現在のテキスト表示
			scoreText.text = "ポイント：" + point; 				//point変数が変化するので、毎フレームごとにpointの値を読み込んだ文章を表示する
			...
			if (zoyamaImg.x >= 400) {						//画面端にぞう山画像が行ってしまったら
				...
				//ゲームオーバー後のテキスト表示
				gameOverText.text = "GAMEOVER 記録：" + point + "ポイント";				//テキストに文字表示 
			}
